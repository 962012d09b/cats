...
class _AlertCountPieChartState extends State<AlertCountPieChart> {
  ...
  void _createOverlayEntry(int touchedSectionIndex) {
    ...
    final tooltipToDisplay = switch (touchedSectionIndex) {
      0 => _tpTooltip,
      1 => _fpTooltip,
      2 => _unknownTooltip,
      _ => null,
    };
    if (tooltipToDisplay == null) return;
  ...
  RichText _generateTooltipText(String alertName, Map<String, int> relevantData) {
    ...
    contentSpans = relevantData.entries.map((e) {
      return TextSpan(
        children: [
  ...
  Widget build(BuildContext context) {
    final allTps = widget.data.results.alertCountPerAlertType.tp.entries.where((e) => e.value > 0).toList()
      ..sort((a, b) => b.value.compareTo(a.value));
    final allFps = widget.data.results.alertCountPerAlertType.fp.entries.where((e) => e.value > 0).toList()
      ..sort((a, b) => b.value.compareTo(a.value));
    final allUnknowns = widget.data.results.alertCountPerAlertType.unknown.entries.where((e) => e.value > 0).toList()
      ..sort((a, b) => b.value.compareTo(a.value));
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: widget.screenSize == ScreenSize.large
                        ? Theme.of(context).textTheme.bodyMedium
                        : Theme.of(context).textTheme.bodySmall,
                  ),
                  Text(

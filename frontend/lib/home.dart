  ...
  Widget build(BuildContext context) {
    ...
    if (Uri.parse(defaultHost).host == currentUri.host && defaultPort == currentUri.port) {
      return Tooltip(
        ...
        message:
            "Backend operates in read-only mode.\nAll modifying actions (editing or adding datasets/modules, saving, loading) will result in an error.\n\nSince said backend runs on a non-standard port, your device may not be able to connect to it if you are using a web proxy.\n\nVisit the GitHub repo (top-right corner) to set up your own backend in 10(-ish) seconds!",
          ...
          children: [
            ...
            RichText(
              text: TextSpan(
                ...
                children: [
                  TextSpan(text: "Using demo backend - "),
                  TextSpan(
                    text: "WEB PROXIES MAY CAUSE ISSUES",
            ...
            CircleAvatar(
              ...
              child: Icon(Icons.question_mark, size: 16, color: Theme.of(context).colorScheme.error),
            ),
          ],
        ),
      );
